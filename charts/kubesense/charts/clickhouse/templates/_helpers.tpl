...
{{- include "common.images.pullSecrets" (dict "images" (list .Values.image (dict "pullPolicy" (tpl .Values.volumePermissions.image.pullPolicy .))) "global" .Values.global) -}}
{{- end -}}
...
{{- end -}}
{{- if and (not .Values.keeper.enabled) (not .Values.zookeeper.enabled) (not .Values.externalZookeeper.servers) (ne (int (tpl .Values.shards $)) 1) (ne (int (tpl .Values.replicaCount $)) 1) -}}
...
{{/*
Convert memory resource strings to bytes.
*/}}
{{- define "clickhouse.memory.toBytes" -}}
{{- $value := . -}}
{{- $lastTwo := substr ((sub (len $value) 2) | int) ((add (len $value) 2) | int) $value -}}
{{- $number := substr 0 ((sub (len $value) 2) | int) $value -}}
{{- if eq $lastTwo "Ki" -}}
  {{- mul $number 1024 | int64 -}}
{{- else if eq $lastTwo "Mi" -}}
  {{- mul $number 1048576 | int64 -}}
{{- else if eq $lastTwo "Gi" -}}
  {{- mul $number 1073741824 | int64 -}}
{{ else }}
  {{ $number }}
{{- end -}}
{{- end -}}
